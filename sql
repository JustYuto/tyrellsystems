...
-- Instead of an using LEFT JOIN, it'd be better to use an INNER JOIN to improve performance by not including unnecessary data.
FROM jobs Jobs
...
-- It’s not necessary to look up ‘キャビンアテンダント’ in all of columns, such as location and activity. 
-- Using % at the beginning of ‘キャビンアテンダント’ may slow down processing speed because the database may not be able to use the index.
